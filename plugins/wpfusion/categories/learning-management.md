...
WP Fusion integrates with SureDash to connect your community portal with your CRM. When users register via SureDash, they’ll be added to your CRM with all their registration data.
...
Using WP Fusion and MasterStudy LMS, you can restrict access to your courses and other content based on a user’s tags in your CRM or email marketing platform, as well as apply tags based on student progress, and auto-enroll students into courses based on changes in tags.

### Access Control

For more information on controlling access to MasterStudy courses and lessons based on CRM tags, see the documentation on WP Fusion’s access control features.
...
WP Fusion settings when editing a MasterStudy lesson
...
Using WP Fusion with Parent & Student Access For LearnDash by HonorsWP, you can automatically link student contact records to parent contact records in your CRM or email marketing system.
...
This integration requires the Uncanny Automator plugin.
...
For more information on WP Fusion’s Uncanny Automator integration, see the Uncanny Automator documentation.
...
WP Fusion includes an integration with the Memberoni theme by The Member Site Academy, allowing you to apply tags in your CRM as students progress through courses and lessons.
...
Uncanny LearnDash groups also allows you to create pre-configured group license products which can contain multiple group courses.
...
Using WP Fusion and WPComplete you can apply tags in your CRM when students mark content as completed, or when they complete entire courses.

### Lessons

When WPComplete is active, an additional setting will appear at the bottom of the WP Fusion meta box while editing any post. Here you can specify tags to be applied when all items on the page are marked complete.
...
2. Tags applied in your CRM by AccessAlly are immediately communicated to WP Fusion (for example to award an achievement or trigger a push notification), without requiring a webhook.
...
Since AccessAlly 4.0, it’s possible to use AccessAlly without a CRM. In this scenario, you can use AccessAlly as just a membership plugin, and use WP Fusion for the integration between AccessAlly in your CRM.
...
Using WP Fusion and LearnPress LMS, you can restrict access to your courses and other content based on a user’s tags in your connected CRM.
...
Any custom fields added to your LearnPress registration or profile forms will be automatically detected by WP Fusion and will be listed under the LearnPress section in the Contact Fields settings.
...
Based on our experience with the transition, we’ve decided to discontinue support for eLearnCommerce with WP Fusion, and we’re recommending our customers to consider an alternative LMS solution, such as LifterLMS or LearnDash.

### Overview

Using WP Fusion and the eLearnCommerce LMS by Grow Learn Teach, you can restrict access to your courses, offers, videos, ebooks, and other content based on a user’s CRM tags.
...
- Capture customer information on the LifterLMS checkout and send abandoned cart reminders if they don’t finish checking out
...
This can be used to create dynamic learning paths, with courses personalized to each student. For more info and an example, see our post on Adaptive Learning.
...
If you’re using the Abandoned Cart addon, you will also see an option to apply tags when a student abandons a cart with this access plan in it.
...
WP Fusion enables syncing custom fields and progress data between LifterLMS and your CRM.
...
- Membership Status: When a user joins a membership, this will be synced as enrolled. When a user leaves a membership, it will be synced as unenrolled.
...
WP Fusion’s Abandoned Cart addon (available to Plus and Professional license holders) can capture customer details on the LifterLMS checkout form and sync them to your CRM before checkout has been completed.
...
WP Fusion’s Abandoned Cart addon lets you tag LifterLMS customers in your CRM when they start, but don’t finish, checking out on your store.
...
For more information on the available options with abandoned cart tracking and LifterLMS, see the Abandoned Cart Tracking documentation.

### Enhanced Ecommerce

WP Fusion’s Enhanced Ecommerce addon (available to Plus and Professional license holders) can sync LifterLMS orders to supported CRMs and email marketing platforms.

Deep Data from LifterLMS showing on the contact record in ActiveCampaign
...
{last_completed_step}: The title of the last lesson or quiz completed in the course.
...
{grade}: The student’s quiz grade.
...
- LifterLMS memberships meta: Syncs any enabled membership fields (like Level Name, Start Date, and Status) to the corresponding custom fields in your CRM. Does not modify any tags.
...
Using WP Fusion and the Sensei addon for WooCommerce, you can restrict access to Lessons, and Topics based on a user’s CRM tags.
...
- Unenroll from Course when Linked Tag is Removed: By default, WP Fusion will unenroll a user from a course when their linked tag is removed. However, if you are using drip-released content, this could cause a payment failure to remove the user from the course, and when they re-enroll the content drip will start over from the beginning. Disabling this setting will leave the user enrolled in the course when the linked tag is removed. In this case, it’s recommended to also protect access to the course with a tag, so users can’t access the course until they’ve reactivated their subscription.
...
- Field – Course Enrollment Expiration Date: If your courses are set to exipre, this field will be updated with the student’s course access expiration date (at the time they enroll in the course).
...
Lessons and topics can be shown as disabled in the course navigation using the **Lock Lessons** setting.
In this screenshot, the second lesson is locked by WP Fusion. This is using the default LearnDash theme.
Locked lessons and topics are also disabled in focus mode (using the default LearnDash theme).
...
The Locked Lessons feature also works with the BuddyBoss app. In that case, the lesson will be displayed as disabled.

Lessons locked by WP Fusion’s “Lock Lessons” setting are displayed as disabled in the BuddyBoss app.
...
- Field – Category Scores: If your quiz has question categories, you can sync the score for each question category to a separate custom field in your CRM.
...
In addition to syncing course progress on individual courses, WP Fusion adds several “pseudo” fields that enable syncing a student’s overall LearnDash progress to custom fields in your CRM. This can be configured under Settings » WP Fusion » Contact Fields.
...
Event tracking can also be configured globally at Settings » Event Tracking.
If your CRM supports multi-key events, you will see additional options for syncing event data.
...
The LearnDash course progress meta batch operation syncs any enabled progress meta fields (Last course enrolled, last course completed, last course completed date, course completion percentages, etc.) to the selected custom fields in your CRM. It does not apply any tags or affect enrollments.
