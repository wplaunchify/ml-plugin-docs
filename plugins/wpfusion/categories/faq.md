...
Staging mode can be enabled from the Advanced settings tab.
...
Similar to WooCommerce Subscriptions, WP Fusion will detect when your site URL changes, and if the site URL is different from the URL where WP Fusion was installed, staging mode will automatically be activated.
...
To do that it sets a cookie, which can be blocked by caching. If you have trouble with Return After Login, try adding a cache exclusion for the wpf_return_to cookie.
...
- wpf_guest: Used with identifying guest form submissions to the tracking scripts.
- wpf_leadsource: Used with lead source tracking.
- wpf_ref: Used with lead source tracking.
- wpf_return_to: Used with return after login.
- wpfrc: Used with abandoned cart recovery.
...
Please note that if you are using a platform that supports site tracking, you will need to obtain permission from your visitors before tracking their page views.
...
Workflows are required to automatically send list assignment updates back to WordPress via webhooks. Contact property updates can still be synced back to WordPress automatically by configuring a webhook in a private app.
...
- ✨ New Added Document Library Pro integration for syncing lead capture form submissions to the CRM with per-document tagging
...
- ✨ New Added SureDash integration for syncing course and lesson completions
...
- ✨ New Added a SureForms integration
...
- ✨ New Added support for segmenting using Labels with Pipedrive
...
- ✨ New Added beta support for FluentCart
...
- ✨ New Added support for syncing additional fields from the Event Tickets Additional Fields extension
...
- ✨ New Added - New "LearnDash Quiz Results" batch operation to sync quiz scores, points, and category scores to the CRM
...
- ✨ New Added a setting to apply tags based on LifterLMS order statuses
...
- Updated Fluent Community integration to move settings to individual Spaces and Courses
...
- ✨ New Added WooCommerce Subscriptions global tag settings to apply tags when a user has any active subscription
...
- ✨ New Added an integration with WP Travel Engine
...
- ✨ New Added a Sender.net CRM integration
...
- ✨ New Added a ClickWhale integration
...
- ⚡️ Improved Added support for field mapping using the generic field mapping UI in Gravity Forms
...
- ✨ New Added Booking Rescheduled trigger to FluentBooking integration
...
- ✨ New Added support for typing new segment names into the "Select List(s)" dropdown when using a multiselect field for segmentation with HubSpot
- ✨ New Added multiselect (values) field type for syncing MemberPress multiselect data as their values, instead of their labels
- ⚡️ Improved If a user accepts a retention offer to skip a renewal payment with Cancellation Survey and Offers for Woo Subscriptions, the updated subscription fields will now be synced to the CRM
...
- ✨ New Added an integration with the Elementor Hotspots element
- ✨ New Added support for Elementor display conditions
- ✨ New Added "Required tags (all)" option to the Divi visibility settings
...
- ✨ New Added a BookingPress integration
...
- ✨ New Added support for tickets and RSVPs configured as blocks with Event Tickets
- ✨ New Added support for a backup Service Account Key for Infusionsoft/Keap. If the primary key is throttled, WP Fusion will automatically switch to the backup key until 12am UTC
- ✨ New Added option to update existing user's tags and metadata when importing users via the import tool
- ⚡️ Improved The Memberoni integration will now remove any configured tags on the lesson, roadmap step, course, and/or roadmap when a user marks a lesson or roadmap step as incomplete, or when a course/roadmap progress is reset
...
- ✨ New Added ability to sync MemberPress subscription and transaction fields to separate CRM fields on a per-product basis
...
- ⚡️ Improved With Event Tickets Plus, if an attendee's email address is changed on the frontend, and "Create New Attendees on Edit" is enabled, a new contact record will be created in the CRM for the new email address and any tags for the event will be applied
...
- ✨ New Added support for retrying failed API calls in the logs
...
- 🛠️ Dev The Secure Block for Gutenberg has been updated to apiVersion 3.0
...
- ✨ New Added support for multi-level donation forms in the new GiveWP visual form builder
...
- ✨ New Added support for Presto Player Email Capture forms
...
- ✨ New Added a link to the CRM contact record in the attendee details modal with Event Tickets
...
- 🔧 Fixed Fixed HighLevel integration not saving the location ID during initial setup when using the older API-key based setup method
...
- ✨ New Added support for applying lists in supported CRMs from FluentBooking bookings
- ✨ New Added support for creating and updating Accounts along with contacts in ActiveCampaign
...
- ✨ New Added support for FluentCommunity
...
- ✨ New Added ability to apply tags to Amelia event bookings in addition to appointments
...
- ✨ New Added option to apply tags when a user accepts a retention offer, and sync the offer title to the CRM, with the WooCommerce Cancellation Surveys plugin
- ✨ New Added option to prevent linked tags from unenrolling users from LearnDash courses in cases of payment failures
...
- ✨ New Added support for Klaviyo webhooks
- ✨ New Added ability to set a default Record Type for new Salesforce contacts created by WP Fusion
- ✨ New Added option to switch between tag IDs and slugs in the FluentCRM (REST API) integration. Tag IDs will be used by default for new installs.
- ✨ New Added Salesforce compatibility to the api.php webhook endpoint
...
- ✨ New Added a Cancellation Survey for WooCommerce Subscriptions integration
- ✨ New Added support for displaying Elementor widgets dynamically based on the access rules configured on the underlying post
- ✨ New Added support for displaying Bricks widgets dynamically based on the access rules configured on the underlying post
...
- ✨ New Added support for webhooks configured via private apps in HubSpot (works with all HubSpot plans)
...
- ⚡️ Improved The new Infusionsoft/Keap API does not support refunding orders, so after a refund WP Fusion will record an order note with a link to the order so it can be refunded manually
...
- ✨ New Added support (via code snippet) for syncing lead source data when updating a contact, instead of just when adding a new contact
...
- 🛠️ Dev Added wpf_api_{$method_name} filter to allow bypassing / overriding API calls in the CRM classes
...
- ✨ New Added support for setting a primary connection for field mapping with WPForms, to make it easier to apply tags via conditional logic
...
- ✨ New Added support for creating and updating Leads from form submissions with Zoho
- ⚡️ Improved When using the api.php webhook method, you can now define a custom ABSPATH via php.ini or a bootstrap file (for custom WP directory locations)
- ⚡️ Improved Improved support for syncing country and region codes with Infusionsoft/Keap
- ⚡️ Improved With the user_meta shortcode, input strings of 8 characters or less will no longer be treated as timestamps (allows for dates like 2024 or 20240101 to be formatted correctly as dates)
- Improvements to the add_object() method in the CRM base class
- ⚡️ Improved When a user is auto-enrolled into an AffiliateWP group via a linked tag, their affiliate account will be automatically set to active
...
- ✨ New Added support for webhooks (aka "outbounds") with KlickTipp
- ✨ New Added a visibility indicator on elements protected by WP Fusion access rules in the Bricks editor
- ✨ New Added AffiliateWP - Referrals batch operation for exporting historical referral data
...
- ✨ New Added a MemberDash integration
...
- ⚡️ Improved With Ontraport lead source tracking, the any enabled lead source fields will be synced to the corresponding Last Referrer fields when a contact is updated
...
- ✨ New Added ability to sync WooCommerce Memberships status and expiration date to separate custom fields per membership plan
...
- 🔧 Fixed Fixed the "Duplicate and Delete" feature for email address changes not working with Mailerlite
...
- ✨ New Added support for Encharge webhooks
- ✨ New Added support for Drip site tracking (was removed at the end of 2022 but Drip has re-enabled it)
...
- 🔧 Fixed Fixed auto-applied coupons with WooCommerce not working on subscription purchases when the current cart total was 0
...
- ✨ New Added a Content Control integration
...
- ✨ New Added support for resubscribing unsubscribed contacts with ActiveCampaign (supports Gravity Forms and Elementor Forms)
- ✨ New Added Apply Tags and Link With Tag settings for individual Teams for WooCommerce Memberships teams
- ✨ New Added option to skip already processed entries when running the Event Tickets Attendees batch operation
...
- ✨ New Added an integration with the ZealousWeb payment plugins for Contact Form 7
...
- 🔧 Fixed Fixed fatal error calling methods that aren't in the WPF_Staging class, while in staging mode (i.e. Argument #1 ($callback) must be a valid callback, class WPF_Staging does not have a method "get_connection_id")
- ✨ New Added Event Variation SKU field for sync with FooEvents

#### #3.43.4 - 4/8/2024

- ✨ New Added ability to link to individual entries in the activity logs
...
- ✨ New Added support for importing all contacts from the CRM via the import tool (rather than a specific segment), with selected platforms
- ⚡️ Improved While in staging mode you can now resync the lists of available tags and fields from the CRM
...
- ✨ New Added a MasterStudy LMS integration
- ✨ New Added option to automatically remove MemberPress corporate account tags from sub-account members when the parent membership is cancelled
- ⚡️ Improved When using HTTP API logging, the amount of time to perform the API call will be recorded to the logs
...
- ✨ New Added a visibility indicator to protected elements with the WPBakery frontend editor
- ✨ New Added a batch export tool for Elementor Forms entries
...
- ✨ New Added support for If-So block editor conditions
...
- ✨ New Added a LatePoint integration
- ✨ New Added an option with Klaviyo to subscribe contacts to lists with marketing consent
...
- ✨ New Added an Amelia booking integration
- ✨ New Added Customer.io site tracking scripts
- ✨ New Added Customer.io webhooks
...
- ⚡️ Improved The JavaScript-based lead source tracking method no longer requires jQuery
...
- 🔧 Fixed Fixed security vulnerability in the user_meta_if shortcode, an editor could potentially execute arbitrary PHP code by passing a function name to the field_format parameter

#### #3.42.9 - 1/30/2024

- ✨ New Added support for the new FluentCRM event tracking module (same site and REST API)
...
- ✨ New Added a CRM integation with Omnisend
...
- ✨ New Added support for the Paid Memberships Pro Gift Membership Addon
- ✨ New Added an order status column to the Easy Digital Downloads orders table
- ✨ New Added Apply Tags - Resumed setting to the MemberPress integration
...
- ✨ New Added support for double opt-ins with Brevo
- ✨ New Added support for setting a default opt-in status with Groundhogg (REST API)
...
- ✨ New Added a SureMembers integration
- ✨ New Added support for using an API key with HighLevel instead of OAuth
...
- ✨ New Added support for EDD Cancellation Survey
- ✨ New Added an option to apply the current user's CRM tags as CSS classes to the HTML <body> element
...
- ✨ New Added an integration with Forminator forms
- ✨ New Added an integration with WP Software License for WooCommerce
- ✨ New Added support for linked tags with Gamipress ranks
- ✨ New Added compatibility with Advanced Shipment Tracking Pro (fork of WooCommerce Shipment Tracking)
- ⚡️ Improved With Fluent Forms and Lead Source Tracking, if the lead source cookies are set, the form will no longer be processed asynchronously (to avoid losing the lead source data)
...
- 🔧 Fixed Fixed Filter Course Steps with LearnDash not working with lesson / topic pagination in the course navigation
...
- ✨ New Added an integration with WP User Manager
...
- ⚡️ Improved If you add define( 'WPF_STAGING_MODE', false ); to wp-config.php, this will disable automatic staging site detection
...
- ✨ New Added support for Event Tracking with FluentCRM (REST API and same site)
- ✨ New Added support for webhooks in Mailchimp journeys
...
- ⚡️ Improved The inline scripts to handle conditional logic on Gravity Forms form fields will now only be loaded if the form uses WP Fusion's conditional logic (thanks @karlemilnikka)
...
- ✨ New Added setting Link with Tag - Affiliate Activation to the Solid Affiliate integration
- ✨ New Added Order Total, Transaction Status, and Subscription Status fields for sync with MemberPress
...
- ✨ New Added options for applying tags to AffiliateWP affiliates based on status
...
- 🛠️ Dev You can now return 0 from the []wpf_get_user_id filter](https://wpfusion.com/documentation/filters/wpf_get_user_id_filter/) to prevent WP Fusion from looking up a user ID based on contact ID
- 🛠️ Dev The wpf_get_user_id filter will now only run before the database query, instead of both before and after

#### #3.41.41 - 10/23/2023

- ✨ New Added a GeoDirectory integration
...
- ✨ New Added a FluentBooking integration
...
- 🛠️ Dev Moved WooCommerce refund actions to the woocommerce_order_fully_refunded hook for compatibility with the WooCommerce Subscriptions - Cancel on Refund extension
...
- ✨ New Added support for LearnDash quiz question categories
- ✨ New Added support for event check-ins with FooEvents
- ✨ New Added a Parent and Student Access for LearnDash integration
- ✨ New Added an integration with Formidable Forms payments (trigger WP Fusion actions based on payment status)
...
- ✨ New Added "Hide checkbox if consented" setting with WooCommerce and Easy Digital Downloads: if the customer has previously consented to marketing, the email optin box can be hidden
...
- ✨ New Added order status tagging with Easy Digital Downloads
...
- ✨ New Added a Blockli Streamer integration
...
- ✨ New Added option to use a multi-select instead of static lists with HubSpot
- ✨ New Added support for tagging based on offline donations with GiveWP
...
- ✨ New Added support for multi-value lookup fields with Dynamics 365
- ✨ New Added option to sync with Cases instead of Contacts with Dynamics 365
- ✨ New Added WP Fusion status column and filters to the MemberPress transactions list
- ✨ New Added WP Fusion status section to the MemberPress single transaction page
- ⚡️ Improved With WordPress 6.3+ the Bento tracking script will now be loaded async by default
...
- ⚡️ Improved If a MemberPress subscription is cancelled, Remove Tags is checked, and the user still has another active subscription to a membership that applies the same tags, those tags will not be removed
...
- ⚡️ Improved Improved styling for locked LearnDash lessons and topics when using the BuddyBoss theme and the Lock Lessons feature
- ⚡️ Improved Improved HubSpot error handling
...
- ✨ New Added option to subscribe ConvertKit subscribers to a form when applying tags
- ✨ New Added option to use a multi-select for segmentation with Zoho (instead of tags)
...
- ✨ New Added a MemberPress Courses integration
- ✨ New Added a UsersWP integration
- ✨ New Added support for automatically embedding the MailerLite tracking script
...
- ✨ New Added a Download Manager integration for protecting downloadable files
- ✨ New Added a dotted line around widgets protected by WP Fusion access rules in the Elementor editor
...
- ✨ New Added course enrollment date and course enrollment expiry date fields for sync with LearnDash
...
- ✨ New Added a tool for exporting historical Fluent Forms entries
- ✨ New Added an option to apply tags based on assistant to the Salon Booking integration
- ⚡️ Improved Force killing the background worker will now also unlock the process lock so a new operation can be started
...
- ✨ New Added a Modern Events Calendar Zoom Integration addon integration
- ✨ New Added support for multiple parameters using the IN operator with the user_meta_if shortcode
- ✨ New Added an ALL operator to the user_meta_if shortcode
- ✨ New Added subscription status, start date, end date, and next payment date fields with EDD Recurring Payments
...
- ⚡️ Improved With EDD Recurring Payments, if Remove Tags is checked, the tags will no longer be removed as soon as the subscription is canceled, they will be removed once the subscription's access period expires
...
- ✨ New Added Filter Course Steps feature for LifterLMS
- ✨ New Added a LifterLMS Memberships Meta export tool
...
- ✨ New Added support for Drip webhooks with the api.php webhook endpoint
- ⚡️ Improved The LearnDash Locked Lesson text now works with the BuddyBoss app
...
- ✨ New Added support for AccessAlly 4.0 and up: WP Fusion can now be used when AccessAlly is in "AccessAlly Managed" mode
- ✨ New Added Membership Level Name and Membership Status fields for sync with LifterLMS
...
- ⚡️ Improved Updated Sendlane integration to use the v2 API (requires generating a new access token)
...
- ✨ New Added Ortto CRM integration
- ✨ New Added option to completely remove all WP Fusion settings from the database when resetting the main settings page
...
- 🔧 Fixed Fixed not being able to sync active and unsubscribed text values to set the opt-in status with MailerLite
...
- ✨ New Added a WooCommerce Product Options integration
- ✨ New Added option to refresh a user's tags when access is denied
- ✨ New Added an Object Sync for Salesforce integration
...
- ⚡️ Improved You can now set the WPF_MULTISITE_PREFIX_KEYS in wp-config.php to turn on blog ID prefixes for usermeta keys
...
- ✨ New Added a Pretty Links integration for tracking link engagement in your CRM
- ✨ New Added a ThirstyAffiliates integration for tracking link engagement in your CRM
- ✨ New Added a WP All Import integration for syncing generated passwords with your CRM
...
- ✨ New Added a WooCommerce Gravity Forms Product Add-ons integration
- ✨ New Added ability to apply tags when a new topic or reply are posted in bbPress (and BuddyBoss) forums and topics
- ✨ New Added support for updating lookup fields and associated entities with Microsoft Dynamics 365 Marketing
...
- 🛠️ Dev If the background worker / exporter goes rogue, you can now hard cancel everything by appending &wpf-cancel-batch to the WPF settings page URL
...
- ✨ New Added wpf-refresh query string parameter to force a refresh of the user's tags and/or meta from the CRM
- ✨ New Added a setting to apply a tag when a specific checkout step is completed in a CartFlows flow
...
- ⚡️ Improved Using the new V2 MailerLite API, if you attempt to create an Unsubscribed subscriber, no data will be sent to MailerLite
...
- ✨ New Added an integration with YITH WooCommerce Booking
- ✨ New Added an integration with the Virtual Events addon for The Events Calendar
- ✨ New Added a sync_if_empty parameter to the user_meta shortcode
...
- ⚡️ Improved Improved: When syncing an optin from a supported ecommerce plugin to MailerLite, if the subscriber's status is active or unconfirmed it will not be changed
...
- ✨ New Added SureCart integration
...
- ✨ New Added support for WooCommerce High Performance Order Storage
- ✨ New Added Optin Status field for sync with MailerLite — you can sync a value of active, unconfirmed, or unsubscribed to update the subscriber's status in MailerLite
...
- ✨ New Added Breakdance Builder integration
...
- ✨ New Added Engage CRM integration
...
- ✨ New Added Intercom event tracking
- ✨ New Added Intercom site tracking
- ✨ New Added Klaviyo event tracking
...
- 🛠️ Dev Added filter wpf_event_tickets_apply_tags
...
- ✨ New Added support for AffiliateWP referrer tracking with Fluent Forms
- ⚡️ Improved With the WooCommerce Subscriptions Gifting integration, if a separate shipping address was provided by the customer at checkout, the shipping details will be synced to the gift recipient's contact record
...
- ✨ New Added Group Leader Email and Group Name fields for sync with the Restrict Content Pro Group Accounts addon
...
- ✨ New Added Pipedrive CRM integration
...
- ✨ New Added event check-in support to Modern Events Calendar integration
...
- ✨ New Added Thrive Automator integration
- ✨ New Added Thrive Apprentice integration
- ✨ New Added LearnDash Progress Meta batch operation
...
- ✨ New Added tag-based conditional logic to Gravity Forms form fields
- ✨ New Added support for syncing custom profile and registration fields with LearnPress
- ✨ New Added an integration with the Modern Events Calendar RSVP Addon
...
- 🔧 Fixed Fixed fatal error trying to delete import groups that contained WP_Errors
...
- ✨ New Added Holler Box integration
...
- ✨ New Added an integration with the BuddyBoss App's new Access Controls component
...
- ✨ New Added support for WP Event Manager's Sell Tickets Addon
...
- ✨ New Added an integration with WPBakery Page Builder
- ✨ New Added an integration with WooCommerce Payments
...
- ✨ New Added Bricks builder integration
...
- 🔧 Fixed Fixed WooCommerce automatic discounts not applying when logging in using the checkout login form
...
- ✨ New Added the_excerpt shortcode for use in the restricted content message
- ✨ New Added Current Page pseudo-field to lead source tracking fields
...
- 🛠️ Dev Added parameters $event_id and $ticket_id to the wpf_event_tickets_attendee_data filter
- 🛠️ Dev Added filter wpf_loaded_tags when tags are loaded from the CRM for a user

#### #3.40.6 - 5/23/2022

- ✨ New Added option to "skip already processed" to the Event Tickets attendees batch operation
...
- ✨ New Added WP Booking System integration
- ✨ New Added support for Sendinblue event tracking
- ✨ New Added support for Sendinblue site tracking
- ✨ New Added the ability to restrict the purchase of LifterLMS access plans using tags
...
- ✨ New Added Add Attendees option to EventON integration
...
- ✨ New Added EMPTY and NOT EMPTY comparisons to the user_meta_if shortcode
...
- ✨ New Added Constant Contact integration
...
- ✨ New Added WPPayForm integration
- ✨ New Added option to use a custom picklist field for tags with Salesforce
...
- 🔧 Fixed Fixed missing second parameter $force in wpf_get_tags() function
...
- ✨ New Added option to completely disable the access control system
- ✨ New Added a WP Fusion status metabox when editing a single WooCommerce subscription
...
- ✨ New Added setting Remove Tags - Cancelled to Teams for WooCommerce Memberships integration
...
- 🛠️ Dev Developers - Added wpf_admin_override filter

#### #3.38.43 - 2/21/2022

- ✨ New Added Microsoft Dynamics 365 CRM integration
- ✨ New Added SliceWP integration
- ✨ New Added Remove Tags and Apply Tags - Cancelled settings to Events Manager integration
- ⚡️ Improved Improved (event tracking) - If an event value is sent to Bento as a valid JSON string then that will be used in place of the default name and val properties in the details of the event payload
...
- ✨ New Added order sync status column to WooCommerce orders list table
...
- ✨ New Added Solid Affiliate integration
...
- ⚡️ Improved Staging site detection will now prompt you whether to recognize the new site URL as the main site vs. staying in staging mode (similar to WooCommerce Subscriptions)
...
- ✨ New Added ARMember integration
- ✨ New Added Apply Tags - Check-in setting to Event Tickets integration
...
- ✨ New Added support for syncing user profile data from Advanced Custom Fields flexible content fields
- ✨ New Added Apply Tags - Pending option to MemberPress integration
- ✨ New Added Link with Tag functionality to LearnPress integration
...
- ✨ New Added User Menus integration
- ✨ New Added Status section to WP Fusion settings with status and debug information about the plugin
- ✨ New Added support for Group and Repeater fields with Advanced Custom Fields
...
- ✨ New Added support for Fluent Forms User Registration
- ✨ New Added option to sync WooCommerce Subscriptions details to separate custom fields in the CRM for each subscription product
...
- ✨ New Added a new api.php endpoint that can be used for super fast async webhook processing
...
- ✨ New Added support for custom objects with HubSpot
...
- ✨ New Added timezone-offset attribute to user_meta shortcode
...
- 🛠️ Dev Developers - The active CRM object is now passed by reference via the wp_fusion_init_crm action and can be operated on

#### #3.38.23 - 11/8/2021

- ✨ New Added IN and NOT IN comparisons to the user_meta_if shortcode
...
- ✨ New Added WooCommerce Payment Plans integration
...
- ✨ New Added Download Monitor integration
- ✨ New Added BuddyBoss group organizer linked tag option
...
- ✨ New Added Bento marketing automation integration
...
- ✨ New Added If-So Dynamic Content integration
- ✨ New Added support for syncing the Zoom meeting ID and join URL with FooEvents
...
- ✨ New Added RestroPress integration
- ✨ New Added Import Trigger tag option for Jetpack CRM
- ✨ New Added option to sync LearnDash quiz scores to a custom field in the CRM
...
- ✨ New Added email optin checkbox and optin tagging for Easy Digital Downloads
- ✨ New Added support for FluentCRM webhooks
- ✨ New Added email optin setting to GiveWP integration
...
- ✨ New Added event category tagging for Events Manager events
...
- ✨ New Added Upsell Plugin integration
- ✨ New Added WooCommerce Memberships for Teams team meta batch operation
...
- 🛠️ Dev Developers - Added action wpf_meta_boxcontent{$post->post_type}
...
- ✨ New Added Event Tickets attendees batch operation
...
- ✨ New Added support for auto-applied discounts with Easy Digital Downloads
...
- ✨ New Added Ninja Forms entries batch export tool
- ✨ New Added PulseTechCRM integration
...
- ✨ New Added support for WooFunnels custom checkout fields
...
- ✨ New Added Tickera integration
- ✨ New Added Give Gift Aid integration
...
- ✨ New Added FluentCRM (REST API) CRM integration
- ✨ New Added WooFunnels integration
...
- ✨ New Added action wp_fusion_init
...
- 🛠️ Dev Developer Updates:
 	...
 	✨ New Added wpf_configuresetting{$setting_id} filter
...
- ✨ New Added preuser{$field} filter on user data being synced to the CRM
...
1. When someone registers for an event in The Events Calendar, Uncanny Automator adds the user to a free LearnDash group
2. Simultaneously, WP Fusion syncs the attendee’s details and the event details to your connected CRM
3. After the event time has passed, your CRM triggers a followup email, which includes a tracking link pointing to a Gravity Forms survey, which WP Fusion pre-fills with the attendee’s information from your CRM
4. When the survey form is submitted, Uncanny Automator notifies the LearnDash group leader that there’s been a new survey entry
