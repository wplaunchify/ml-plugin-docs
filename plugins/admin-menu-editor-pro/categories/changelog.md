...
**Source:** [https://adminmenueditor.com/documentation/changelog/](https://adminmenueditor.com/documentation/changelog/)
...
- Fixed insufficient escaping for the “Admin CSS” setting in the “Tweaks” tab.

Changed
- Access to the “Tweaks” tab now requires the 
```
edit_theme_options
```

 capability, in addition to other AME permissions. This shouldn’t affect most users since administrators normally have this capability by default.

#### 2.33.3 (2026-05-20)

Fixed
- WP 7.0 compatibility: Fixed invisible buttons in the license information screen.
- Fixed a potential issue with upgrading from 2.33 where, if you had previously enabled the “Bottom border” option for menu headings, the headings would get all four borders in 2.33.2, not just a bottom border. Now the widths of other borders should correctly default to zero.
- Fixed long role names making the role sidebar in the “Roles” tab very wide when a user is selected.
- Fixed a minor conflict with Elementor that made the allow/deny icons in the “Content Permissions” panel blue.
- Fixed a number of PHP 8.5 deprecation notices about 
```
SplObjectStorage
```
...
- Hotfix for a cache invalidation bug that caused pre-existing menu heading styles to stop working on sites that also had other custom styles for the admin menu.

#### 2.33.1 (2026-05-17)

Added
- Added default role capability data for WP 7.0.
...
Added
- Reworked and significantly expanded menu heading styles. If you’ve previously set up custom heading styles, you may need to review and update those settings.
- Added the ability to hide more table elements like row actions and the "Bulk Actions" box. You can find these new settings in the "Tables" tab. The plugin detects available elements when you visit the relevant admin page, so if you don’t see the new settings, go to the page that shows the table, then go back to the "Tables" tab.
...
- Added a letter spacing setting to various "Font" panels.
- Added visual side indicators to border width controls.

Fixed
- WP 7.0 compatibility: Fixed a number of layout issues and visual bugs that only appeared in WordPress 7.0.
- Fixed the 
```
[ame-count-bubble]
```

 shortcode not displaying the "WooCommerce -> Orders" bubble from WooCommerce 10.6.0.
- Fixed automatic menu heading text hover color assignment sometimes not working correctly when using a non-default admin color scheme.
- Fixed a few PHP notices like "Using null as an array offset is deprecated, use an empty string instead".
- Fixed a PHP deprecation notice when saving some forms without a role/user selected.
- Fixed the "Priority" dropdown in the "Dashboard Widgets" tab not expanding to the full width of the widget.

Changed
- Renamed the "Columns" tab to "Tables" because it now shows more admin table settings, not just table columns.
- Tested up WP 7.0-RC4.
...
- Removed some metadata files like 
```
package.json
```
...
- Fixed WooCommerce Orders being rendered as 
```
Order – ...
```

. That dash should now appear as an actual dash.
...
- Fixed a minor visual issue where some box backgrounds could overflow box borders when using rounded corners and custom background colours.
...
- Added a filter that can be used to turn off admin menu customizations. Basic example: 
```
add_filter('admin_menu_editor-disable_customizations-admin_menu_structure', '__return_true');
```
...
```
 ...
 variable is not a native array but is still array-like.
...
- Fixed a potential PHP warning about array indexing in 
```
hide-others-posts.php
```

 when at least one of the “Hide Other Users’ Posts” options was enabled in the “Tweaks” tab.
...
- Limitations: This feature only affects post tables like the one shown under “Posts -> All Posts”. Hiding other users’ posts from someone doesn’t prevent them from opening those posts by following a direct link, and they can still edit the posts if they have the necessary capabilities (e.g. 
```
edit_others_posts
```
...
- Fixed a PHP 8 deprecation notice about creating dynamic properties on the 
```
ameRexCapability
```
...
- Note: Since Admin Customizer is a complex and brand-new feature, it may currently have more bugs than the rest of the plugin.
...
- Fixed a rare but severe admin dashboard performance issue that was triggered by dynamically generated stylesheets using the 
```
Content-Length
```
...
Fixed
- Fixed a fatal error in the role editor and on “Settings -> Easy Hide” that could be triggered when a single admin menu item had multiple notification badges. In this context, “badge” refers to the colourful circles that WordPress uses to show things like the number of available updates and pending comments.
...
Fixed
- Fixed a bug where updating Admin Menu Editor Pro and the Branding add-on in a certain order could lead to a crash.
- Fixed the error 
```
Undefined index: menu_styles in ...menu-styler.php
```

. This could show up either as a PHP warning or as a notice depending on the PHP version.
...
- Fixed a fatal error (“array_merge(): Argument #1 must be of type array”) and multiple warnings that were caused by third-party roles that had an invalid capability list. The list should be an 
```
array
```

, but for some custom roles it is apparently 
```
null
```
...
```
 ...
 files that could cause 404 errors for users looking at the developer console.
...
Added
- Optionally, you can make the plugin clear the option cache when updating all subsites in Multisite. This is usually not necessary, but may help with compatibility issues when using an external object cache. Use the new custom filter “admin_menu_editor-clear_role_cache” to enable this feature. Example: 
```
add_filter('admin_menu_editor-clear_role_cache', '__return_true');
```
...
```
 ...
 shortcode not working in login redirects. It would always output “(No user)” instead of the actual user data.
...
- Switched the JSON serialization implementation from the jQuery JSON plugin to 
```
JSON.stringify
```

. While this should not affect most users in any way, those that use Internet Explorer may or may not notice some changes in the handling of Unicode characters (e.g. in menu titles).
...
. These are mainly intended to be used to create dynamic redirects, but they will also work in posts and pages.
...
Fixed
- Added a workaround to deal with plugins and themes that create invalid, non-string capabilities. Previously, if another plugin used a capability like 
```
false
```

 (the boolean value, not the word) when registering a custom post type or taxonomy, the “Roles” tab would be empty and unusable.
...
. They display the current user’s first name, last name, and username/login, respectively.
...
```
 ...
 shortcode. When used in a menu title, it takes the number bubble from the default menu title and displays it in the custom title. A “bubble” is the small number in a colored circle that appears on some menu items, like “Dashboard -> Updates (3)”, “Comments (12)”, or “WooCommerce -> Orders (5)”.
...
- Added a workaround that improves compatibility with plugins that set their menu icons using CSS. Previously, there were situations where users were not able to change menu icons because the plugin that created the menu item used CSS to override the normal icon. AME now adds an extra stylesheet that overrides icons with 
```
!important
```
...
```
 ...
 elements that the plugin generates for menu items configured to open in a frame. Leave it empty to calculate the height automatically (the default).
...
- Added basic WP-CLI support. Run 
```
...
```
 ...
 to set your license key. The plugin will attempt to automatically activate that key the next time an administrator visits the Dashboard. It will only try that *once* – if the key is incorerect or something else goes wrong, you’ll have to activate your key manually.
...
```
 ...
 menu title. Technically, it’s not valid to set the title to 
```
null
```

, but it turns out that some plugins do that anyway.
...
- Update notifications now work even when the plugin installed in 
```
mu-plugins
```

. However, updates still need to be uploaded manually because WordPress does not support automatic updates for MU plugins.
...
- Only display the survey notice on the editor page, and let the user disable it by adding a 
```
never-display-surveys.txt
```
